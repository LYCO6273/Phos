...
def opt_r (lux_r):
    ...
    weights = np.log(2.7*lux_r + 1) * sens
    np.clip(weights, 0, 1, out=weights)
    #根据点光源强度确定权重图
    weights = np.clip(weights, 0, 1)
    #裁切权重图以防超限
    bloom_base = cv2.GaussianBlur(lux_r * weights, (ksize , ksize ), sens * 35)
    #应用高斯模糊
    bloom_effect = bloom_base * weights * strg
    #应用权重

    lux_r = lux_r * 0.9 + bloom_effect * 0.05 - weights * 0.05
    #将光晕效果叠加回底片
...
def opt_g (lux_g):
    ...
    weights = np.log(2.7*lux_g + 1) * sens
    np.clip(weights, 0, 1, out=weights)
    #根据点光源强度确定权重图
    weights = np.clip(weights, 0, 1)
    #裁切权重图以防超限
    bloom_base = cv2.GaussianBlur(lux_g * weights, (ksize , ksize ), sens * 35)
    #应用高斯模糊
    bloom_effect = bloom_base * weights * strg
    #应用权重

    lux_g = lux_g * 0.9 + bloom_effect * 0.05 - weights * 0.05
    #将光晕效果叠加回底片
    return lux_g
...
def opt_b (lux_b):
    ...
    weights = np.log(2.7*lux_b + 1) * sens
    np.clip(weights, 0, 1, out=weights)
    #根据点光源强度确定权重图
    weights = np.clip(weights, 0, 1)
    #裁切权重图以防超限
    bloom_base = cv2.GaussianBlur(lux_b * weights, (ksize , ksize ), sens * 35)
    #应用高斯模糊
    bloom_effect = bloom_base * weights * strg
    #应用权重

    lux_b = lux_b * 0.9 + bloom_effect * 0.05 - weights * 0.05
    #将光晕效果叠加回底片
...
def neg_tone_cyan(lux_r):
    ...
    relative_log = 4 * (0.237190 * np.log10(5.555556 * lux_r + 0.102272) + 0.385537)
    ...
    result = np.clip((0.556 - Pt)*1.792 * 1.13 , 0, 1) #将透光率映射回亮度，反转并裁切到0-1范围
    result = result ** 2.05 #应用伽马校正
    return result
...
def neg_tone_magenta(lux_g):
    ...
    relative_log = 4 * (0.237190 * np.log10(5.555556 * lux_g + 0.102272) + 0.385537)
    relative_log = np.clip(relative_log, 0, 4)  # 裁切到 xp 的对应范围
    density = np.interp(relative_log, xp, fp) #利用插值曲线计算密度
    Pt = 10.0 ** (-density) #根据密度计算透光率
...
def neg_tone_yellow(lux_b):
    ...
    fp = np.array([0.9950, 0.9720, 0.9720, 1.0298, 1.1391, 1.3377, 1.4868, 1.6755, 1.8146,
               1.9636, 2.1126, 2.2715, 2.4106, 2.5298, 2.6093, 2.6689, 2.6887], dtype=np.float32)
    #用于确定曲线的控制点，从Gold200技术文档里扒下来的（笑,而且据说Gold200和富士C200其实是同一个生产线
    
    relative_log = 4 * (0.237190 * np.log10(5.555556 * lux_b + 0.102272) + 0.385537)
    relative_log = np.clip(relative_log, 0, 4)  # 裁切到 xp 的对应范围
    density = np.interp(relative_log, xp, fp) #利用插值曲线计算密度
    Pt = 10.0 ** (-density) #根据密度计算透光率
