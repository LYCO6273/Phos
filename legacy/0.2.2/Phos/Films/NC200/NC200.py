...
def opt_r (lux_r):
    ...
    lux_r = lux_r * 0.85 + bloom_effect * 0.35
    #将光晕效果叠加回底片
...
def opt_g (lux_g):
    ...
    lux_g = lux_g * 0.85 + bloom_effect * 0.22
    #将光晕效果叠加回底片
...
def opt_b (lux_b):
    ...
    lux_b = lux_b * 0.85 + bloom_effect * 0.15
    #将光晕效果叠加回底片
...
def neg_tone_cyan(lux_r):
    ...
    relative_log = 4 * (0.247190 * np.log10(5.555556 * lux_r + 0.072272) + 0.385537)
...
def neg_tone_magenta(lux_g):
    ...
    relative_log = 4 * (0.247190 * np.log10(5.555556 * lux_g + 0.072272) + 0.385537)
    relative_log = np.clip(relative_log, 0, 4)  # 裁切到 xp 的对应范围
    density = np.interp(relative_log, xp, fp) #利用插值曲线计算密度
    Pt = 10.0 ** (-density) #根据密度计算透光率
...
def neg_tone_yellow(lux_b):
    ...
    relative_log = 4 * (0.247190 * np.log10(5.555556 * lux_b + 0.072272) + 0.385537)
    relative_log = np.clip(relative_log, 0, 4)  # 裁切到 xp 的对应范围
    density = np.interp(relative_log, xp, fp) #利用插值曲线计算密度
    Pt = 10.0 ** (-density) #根据密度计算透光率
