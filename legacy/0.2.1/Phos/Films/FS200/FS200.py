...
def neg_tone(lux):
    ...
    log_lux = np.log10(lux + 0.42)  # 转换到对数曝光量，并防止 log(0)
    relative_log = 4.82*log_lux + 1.67   # 校准偏移量
    relative_log = np.clip(relative_log, 0, 5)  # 裁切到 xp 范围
    density = np.interp(relative_log, xp, fp) #利用插值曲线计算密度
    Pt = 10.0 ** (-density) #根据密度计算透光率
    result = np.clip((0.669 - Pt)*1.67, 0, 1) #将透光率映射回亮度，反转并裁切到0-1范围
