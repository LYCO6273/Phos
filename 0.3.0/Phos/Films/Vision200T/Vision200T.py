"""Kodak Vision3 200T (ECN-2 negative) - full datasheet-driven pipeline."""
...
def _char_curves():
    curves = []
    for name in ("b", "g", "r"):
        xp, fp = load_curve_csv(f"vision200t/vision200t_char_curve_{name}.csv")
        curves.append((xp, fp))
    return curves
...
def _dmin_spectrum() -> np.ndarray:
    return interp_on_wl("vision200t/vision200t_dmin.csv", taper_nm=0.0)
...
def process(xyz: np.ndarray,
    ...
    """Vision 200T: spectral layer exposures -> optics -> negative pipeline."""
    ...
    offsets = np.array([-0.207, -0.196, -0.242], dtype=np.float32)
